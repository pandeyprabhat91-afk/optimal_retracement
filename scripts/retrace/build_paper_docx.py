"""Research-format paper -> docs/GPS_LOSS_RETRACE_PAPER.docx

Rewrite for clarity: plain language, every term defined on first use,
combos and pass/fail gates stated once in tables, mechanism behind each
result explained in one line. Same numbers, same figures.
"""
...
OUT = str(ROOT / "docs" / "GPS_LOSS_RETRACE_PAPER.docx")
...
sec0.left_margin = Inches(0.7)
sec0.right_margin = Inches(0.7)
...
def center(t, size=11, bold=False):
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run(t)
    r.font.size = Pt(size)
...
def fig(path, caption, width=5.8):
    doc.add_paragraph().add_run().add_picture(path, width=Inches(width))
...
title("Which Sensors Bring a Drone Home?")
title("GPS-Loss Path Retrace: Real Logs, Perfect Worlds, Live SITL")
center("MTech Capstone Project, GPS-Denied Drone Navigation, IIT Madras", 11, True)
center("September 2026, branch optimal-retracement", 10)
...
h("1. The question in plain terms")
para(
...
h("2. Combos, gates, pass bars")
para(
    "Combos are cumulative: each adds one aid to the previous set. Gates pick clean GPS truth windows (HDop under "
    "1.0, 10+ satellites, speed under 0.3 m/s for calibration). A 30 s run passes Exp1 under 2 percent drift "
...
table(
    ...
    [
        ...
        [
            "C4",
            "+ thrust drag model",
            "rotor drag caps velocity error instead of double-integrating it",
        ],
...
para(
    ...
    "per flight. Two traps: attitude logs in degrees while everything else is radians, and the flights need separate "
    "reporting or pooling hides the mechanism."
)

h("4. Method")
h2("4.1 One core, three testbeds")
para(
...
fig(
    ...
    "Figure 1. Retrace pipeline: calibration, propagation, per-combo aids, Exp1 forward vector and Exp2 reverse pursuit.",
)
h2("4.2 Offline replay (real logs)")
para(
    "For each log, clean GPS windows seed 60 s GPS-denied runs per combo, scored at 10/30/60 s horizons against gated "
    "GPS truth (72 rows total: 6 combos x 3 horizons x 2 flights x 2 experiments). Exp2 flies the stored 2 Hz trail "
    "backwards with combo-derived heading noise. Script: run_matrix.py."
)
h2("4.3 Analytic optimal world (perfect sensors)")
para(
...
h2("4.4 Live SITL square (raw sensors)")
para(
    ...
    "Takeoff is ramped because a step climb shook the sim IMU into a 10 s storm. The estimator node ports the core "
...
h2("4.5 SITL optimal control (perfect sensors, flown geometry)")
para(
...
h("5. Results")
h2("5.1 Perfect world: core pair suffices")
table(
...
h2("5.4 Breadcrumb always lands")
para(
...
h2("5.5 Raw SITL repeats the ranking at 50x scale")
table(
...
h2("5.6 Perfect SITL: 6 mm, case closed")
table(
